...
/// `open`, so stderr is invisible). Path overridable via PICROPHONE_LOG.
let picrophoneLogPath = ProcessInfo.processInfo.environment["PICROPHONE_LOG"] ?? "/tmp/picrophone.log"

func debugLog(_ message: String) {
    let ts = ISO8601DateFormatter().string(from: Date())
    let line = "[\(ts)] \(message)\n"
    if let data = line.data(using: .utf8) {
        if let handle = FileHandle(forWritingAtPath: picrophoneLogPath) {
            handle.seekToEndOfFile()
            handle.write(data)
            try? handle.close()
        } else {
            try? data.write(to: URL(fileURLWithPath: picrophoneLogPath))
        }
    }
}
...
func fail(_ message: String, code: Int32 = 1) -> Never {
    FileHandle.standardError.write(Data("picrophone: \(message)\n".utf8))
    exit(code)
}
...
case "version":
    emit([
        "name": "picrophone",
        "version": picrophoneVersion,
        "protocol": picrophoneProtocol,
...
case "help", "-h", "--help":
    ...
    picrophone \(picrophoneVersion)
    usage: picrophone <command> [options]
